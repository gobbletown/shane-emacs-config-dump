...
(defun docker-container-copy-ip ()
  ...
  (--each
    ...
    (my/copy (chomp (sh-notty (concat "real-docker inspect " it " | jq -r \".[].NetworkSettings.IPAddress\""))))))
...
(defun docker-container-sh (container &optional prefix)
  ...
  (sps
   (concat "docker exec -it " container " sh -c \"/bin/bash || /bin/zsh || sh\"; pak")))
...
(defun docker-container-py (container &optional prefix)
  ...
  (interactive
   (list
    (docker-container-read-name)))
  (sh-notty (concat "docker exec -it " container " pip3 install ptpython"))
  (sh-notty (concat "docker cp /home/shane/.ptpython " container ":/root/.ptpython"))
  (sh-notty (concat "docker cp $SCRIPTS/container-start-ptpython " container ":/root/container-start-ptpython"))
  (sps
   (concat "docker exec -it " container " /root/container-start-ptpython")))
...
(defun docker-container-sh-fmg (container &optional prefix)
  ...
  (interactive
   (list
    (docker-container-read-name)))
  (sh-notty (concat "docker cp $SCRIPTS/container-start-fmg-env " container ":/root/container-start-fmg-env"))
  (sps
   (concat "docker exec -it " container " /root/container-start-fmg-env")))
...
(define-transient-command docker-image-help ()
  ...
  ["Docker images help"
   ...
   ("r" "z-repl run"    docker-image-run-command-selection)
   ("d" "z-repl run default"    docker-image-run-default-selection)
   ...
   ("b" "z-repl run sh"    docker-image-run-sh-selection)
   (";" "z-repl run sh"    docker-image-run-sh-selection)
...
(defun docker-machine-env-selection-edit ()
  ...
  (let ((marked (docker-utils-get-marked-items-ids)))
    (when (/= (length marked) 1)
      (error "Can only set environment vars for one machine at a time"))
    (new-buffer-from-string (sh-notty (concat "docker-machine env " (car marked))))))
...
(defun docker-machine-regenerate-certs ()
  ...
  (let ((marked (docker-utils-get-marked-items-ids)))
    (when (/= (length marked) 1)
      (error "Can only set environment vars for one machine at a time"))
    ;; (term-nsfa (concat "set -xv; nvc -pak docker-machine regenerate-certs " (car marked)))
    (term-nsfa (concat "set -xv; docker-machine regenerate-certs " (car marked) " ; pak"))))
...
(defun docker-image-copy-entrypoint-and-cmd (&optional image)
  ...
  (my-copy (chomp (sh-notty (concat "docker-get-entrypoint-and-cmd " image)))))

;; TODO Make a keybinding for this -- w
(defun docker-image-copy-name-and-tag ()
  (interactive)
  (let ((l (vector2list (tabulated-list-get-entry))))
    (my-copy (concat (nth (tabulated-list-current-column) l)
                     ":"
...
(defun docker-image-copy-cmd (&optional image)
  ...
  (if (not image)
      (setq image (docker-select-one)))
  (my-copy (chomp (sh-notty (concat "docker-get-cmd " image)))))

(defun docker-image-copy-entrypoint (&optional image)
  (interactive)
  (if (not image)
      (setq image (docker-select-one)))
  (my-copy (chomp (sh-notty (concat "docker-get-entrypoint " image)))))

(defun ff-dockerhub (&optional image)
  (interactive)
  (if (not image)
      (setq image (docker-select-one)))
  (sps (concat "ff-dockerhub " image)))

(defun ff-dockerfile (&optional image)
  (interactive)
  (if (not image)
      (setq image (docker-select-one)))
  (sps (concat "ff-dockerhub -d " image)))


(defun my-docker-pull-specific-tag ()
  ""
  (interactive)
  (--each (docker-utils-get-marked-items-ids)
    (sps (concat "my-docker pull -s " it)))
  (tablist-revert))
