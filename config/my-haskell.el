  ...
  (let ((lang
         (mu
          (fz
           (-uniq (str2list
                   (concat
                    (snc "cd \"$NOTES/ws/haskell\"; anygrep -E 'scrape \"LANGUAGE [A-Za-z]+\"' | cut -d ' ' -f 2")
                    ;; (snc "cd \"/home/shane/var/smulliga/source/git/mullikine/glossaries\"; anygrep -E 'scrape \"LANGUAGE [A-Za-z]+\"' | cut -d ' ' -f 2")
                    )))))))
    (if (sor lang)
        (progn
          (beginning-of-buffer)
...
;; For the repl
;; My my-mode map C-a / (beginning-of-line-or-indentation) disables my-mode to run this
...
;; (remove-hook 'haskell-mode-hook 'my/haskell-mode-hook t)

(defun my-enable-intero ()
  (interactive)
  (shut-up
...
(defun ghcid ()
  ...
  (let ((path (get-path)))
    (if (string-match-p "\.hs$" path)
        (sps (concat "ghcid " (q path)) "-p 20 -d")
    ...
    (if lang
        (if imp
            ...
            (cond ((string-equal lang "hs")
                   (sps (concat "x -sh \"ghci -norc\" -e \">\" -s \"import " imp " \" -c m -i")))
                  ((string-equal lang "py")
                   (if (yn "xpti: Use home directory?")
                       (xpti-with-package imp "~")
                     (xpti-with-package imp)))
                  (t (progn (message (concat "please handle this interpreter: " lang))
                            (j 'my-interpreter-import))))
          (progn
            (message (ns "No import detected"))
            (cond ((string-equal lang "hs")
                   (sps "ghci -norc"))
                  ((string-equal lang "py")
                   (if (yn "xpti: Use home directory?")
                  ...
                  (t (progn (message (concat "please handle this interpreter: " lang))
                            (j 'my-interpreter-import))))))
      ;; (cond ((major-mode-p 'haskell-mode) (sps (concat "x -sh \"ghci -norc\" -e \">\" -s \"import " sel " \" -c m -i")))
      ;;       ;; (t (sps (concat "x -sh \"ghci -norc\" -e \">\" -s \"import " sel " \" -c m -i")))
      ;;       )
      )))

(define-key global-map (kbd "H-U") 'my-interpreter-import)



(defun ghcd-info (thing)
  (interactive (list (read-string-hist "ghcd: " (my/thing-at-point))))
  (let ((info (snc (concat "ghcd " (q thing)))))
    (etv info)))
...
(defun haskell-repl ()
  (interactive)
  (comint-quick "haskell-repl" (my/pwd)))
