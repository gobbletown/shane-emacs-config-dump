...
;; v +/";; THis doesnt work" "$EMACSD/config/my-manage-minor-mode.el"
...
(cl-loop for m in modes-for-annotate-mode do
         ;; (message (str m))
         (let ((h (str2sym (concat (sym2str m) "-hook"))))
...
(defun my-annotate-list-buffer-annotations-in-memory ()
...
(defun annotate-load-annotations-around-advice (proc &rest args)
  ;; (my/shut-up (let ((res (apply proc args)))
  ;;               res))
  (my/shut-up (ignore-errors (let ((res (apply proc args)))
                               res)))
  ;; (let ((res (apply proc args)))
  ;;   res)
  )
...
;; (define-key annotate-mode-map (kbd "H-a") 'my-annotate)
...
(defun annotate-path-filter-p (s)
  (try (and (not (string-match-p "^/tmp/" (str (car s)))) (second s))
       t))
...
(defun annotate-actual-file-name ()
  ...
  (substring-no-properties (or (annotate-info-actual-filename)
                               ...
                               (string-or (if (major-mode-p 'eww-mode) (concat "**" (get-path))))
                               (string-or
                                (try
                                 (concat "**" (cond ((major-mode-p 'dired-mode) (slugify (concat (str (buffer-mode)) "-" (mnm (my/pwd)) "-" (buffer-name))))
...
(defun my-annotate-find-buffer-visiting ()
  ;; Making this find virtual buffers would be a nice TODO
  (find-buffer-visiting filename))

;; (defun my-reload-annotate ()
;;   (interactive)
;;   (annotate-mode -1)
;;   (annotate-mode 1))
  ...
  (let* ((filename        (button-get button 'file))
    ...
    (cl-labels ((redraw-summary-window ()    ; update the summary window
                ...
                (update-visited-buffer-maybe ()
                                             (let ((visited-buffer (my-annotate-find-buffer-visiting filename)))
...
(advice-add 'my/revert-kill-buffer-and-window :after 'annotate-kill-buffer-after-advice)
