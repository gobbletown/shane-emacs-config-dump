...
;; pos:len
;; pos:len
(defun make-position-list-function (cmd-filter)
  ...
  (let ((f (func-for-expression
            ...
            `(save-excursion
               ...
               (cl-loop
                ...
                s
                in
                (str2list (cl-sn ,cmd-filter :stdin (buffer-string) :chomp t))
                collect
                (list
                 (progn
                   (search-forward s)
...
;; ;; pos:len
;; ;; pos:len
;; (defun make-position-list-function-from-position-list-command (cmd-filter)
;;   (interactive (list (sed "s/ *#.*//" (fz (cat "$HOME/filters/filters.sh")))))
;;   (let ((f (func-for-expression
;;             "position-list-function"
;;             `(save-excursion
;;                (beginning-of-buffer)
;;                (cl-loop for s in (-drop-last 1 (str2list (sn ,cmd-filter (buffer-string)))) collect (progn (search-forward s) (point))))
;;             t
;;             cmd-filter)))
;;     (add-to-list 'position-list-functions f)
;;     (setq position-list-current-function f)))
...
(defun positions-of-rpl-1 (rpl)
  (-drop-last
   ...
   (mapcar 'byte-to-position (mapcar 'string-to-number (str2list
                                                        (sn
                                                         (concat "rosie grep -w -o jsonpp " (q rpl) " | jq -e \".subs[].s\"")
                                                         (buffer-string)))))))
...
(provide 'my-position-list-navigator)