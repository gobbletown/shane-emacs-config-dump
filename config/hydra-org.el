...
;; This should be a list of tuples; (name, shortcut)
;; Then when I search for them I can be reminded of its shortcut'
(defset org-hydra-snippet-names
  '(src example comment vimrc latex awk snippet equation equationpng jq
        ...
        sps nw sph ghci ghcih-stdout ghcih rh ghciol nohup clojure
...
(defhydra hydra-org-template (:color blue :hint nil)
  ...
  ("*" (hot-expand "<s" "ghcih") "haskell sps ghcih")
  ...
  ("L" (hot-expand "<s" "racket-sublang") "racket")
...
;; org-hydra-snippet-names

;; ;; (defset mytmph '(defhydra
...
;; (nbfo (list2str (mapcar 'third (mapcar 'second (-drop-last 3 (-slice mytmph 3))))))
  ...
  (let (text)
    ...
    (cond ((string-equal mod "racket-sublang")
           (let ((sublang (fz '(racket
                                ...
                                sicp
                                rash))))
             (insert (eval `(bp org-template-gen ,mod ,sublang ,text)))))
          ((string-equal mod "plantuml-svg")
           (insert (eval `(bp org-template-gen ,mod ,(let ((fp (read-string "svg fp: ")))
                                                       (if (re-match-p "\.svg$" fp)
                                                           fp
                                                         (concat fp ".svg")))
                              ,text))))
          (t
           (insert (eval `(bp org-template-gen ,mod ,text)))))))
...
;;          ;; The following will automatically indent the source code
;;          (org-edit-special)
;;          (org-edit-src-exit))))
