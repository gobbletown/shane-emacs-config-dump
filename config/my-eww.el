...
(defset shr-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map [mouse-2] 'my-eww-follow-link)
    ...
    (define-key map "v" 'my-eww-follow-link)
    (define-key map "O" 'shr-save-contents)
    ;; (define-key map "\r" 'shr-browse-url)
    (define-key map "\r" 'my-eww-follow-link)
    map))
...
;; (defun eww-retrieve-around-advice (proc &rest args)
;;   (ignore-errors
;;     (let ((res (apply proc args)))
;;       (deselect)
;;       res)))
;; (advice-add 'eww-retrieve :around #'eww-retrieve-around-advice)
;; (advice-remove 'eww-retrieve #'eww-retrieve-around-advice)

(defun eww-open-file (file)
  "Render FILE using EWW."
  (interactive "fFile: ")
  (orig-eww (concat "file://"
...
(defun my/eww (url &optional use-chrome)
  ...
  ;; This function would error if multiple ewws with the same URL were opened

  ;; Need to use both the url and the current time to make it unique
  ;; I could simply use the current time in the hash. No point anymore in hashing url
...
(defun my/eww-browse-url (&rest body)
...
(defun eww-tag-pre (dom)
  ;; (if (not (string-match-p "Haskell.docset" (str (eww-current-url))))
  ...
  (let ((shr-folding-mode 'none)
    ...
    (insert (eww-fontify-pre dom))
    (shr-ensure-newline)))
;; (string-match-p "Haskmmell.docset" "file:////home/shane/.docsets/Haskell.docset/Contents/Resources/Documents/file/Library/Frameworks/GHC.framework/Versions/8.4.3-x86_64/usr/share/doc/ghc-8.4.3/html/libraries/base-4.11.1.0/Prelude.html#//apple_ref/func/putStrLn")
...
;; (defun eww-buffer-auto-detect-mode () nil)

;; entrypoint
(setq shr-external-rendering-functions '((pre . eww-tag-pre)))
;; (setq shr-external-rendering-functions '((pre)))
          ...
          (let ((found-or-exhausted))
            (while (not found-or-exhausted)
              (let ((pnt (next-single-property-change
                          (point) 'shr-target-id)))
                (message (str pnt))
                ;; (ignore-errors (message shr-target-id))
                ;; (ignore-errors (message url))
...
;; (next-single-property-change (point-min) "Next")
...
;; (defun eww-render-around-advice (proc &rest args)
;;   (ignore-errors
;;     (let ((res (apply proc args)))
;;       (deselect)
;;       res)))
;; (advice-add 'eww-render :around #'eww-render-around-advice)
;; (advice-remove 'eww-render #'eww-render-around-advice)
...
word(s) will be searched for via `eww-search-prefix'."
  ...
  (let ((inhibit-read-only t))
    ...
    ;; (insert (cl-sn (concat "odn curl -s " (q url)) :chomp t))
    ;; (eww-render )
...
;; (define-key eww-mode-map (kbd "c") 'toggle-cached-version)
...
(defun eww-display-html (charset url &optional document point buffer encode use-chrome)
  ...
  (let* ((envs (concat ""
         ...
         (html
          (let ((htmlbuild
            ...
            (if (and eww-use-rdrview
                ...
                (setq htmlbuild (cl-sn (concat "rdrextract -u " (q basepath)) :stdin htmlbuild :chomp t)))
    ...
    (with-current-buffer buffer
      ...
      ;; (new-buffer-from-string (sn (concat "dom-dump " (q "http://google.com"))))
...
(defun reload-eww-use-chrome-dom-matchers ()
  (setq eww-use-chrome-dom-matchers
        (str2list (cat "$NOTES/programs/eww/chrome-dom-url-patterns.txt"))))
...
(defun eww-display-html-around-advice (proc &rest args)
  ...
  (cond
   ((-reduce (lambda (a b) (or a b))
             (mapcar (lambda (e) (string-match-p e (second args)))
  ...
  (let ((url (car args)))
    ...
    (cond ((string-match-p "https?://github.com/.*/issues" url)
          ...
          (t (let ((res (apply proc (cons url (cdr args)))))
               res)))))
;; (advice-remove 'eww-browse-url #'advice-handle-url)
(advice-add 'eww-browse-url :around #'advice-handle-url)
;; (advice-remove 'eww-browse-url #'advice-handle-url)
;; (advice-remove 'eww-browse-url #'eww-browse-url-around-advice)

;; eww takes fewer parameters, but the advice still works
(advice-add 'eww :around #'advice-handle-url)
(advice-add 'my/eww :around #'advice-handle-url)
;; (advice-remove 'eww #'eww-browse-url-around-advice)
;; (advice-add 'eww-open-file :around #'advice-handle-url)
;; (advice-add 'my/eww :around #'advice-handle-url)
;; (advice-remove 'eww-browse-url #'advice-handle-url)

;; (defun my-eww-keyboard-quit ()
;;   (interactive)
;;   (unwind-protect
;;       (keyboard-quit)
;;     (my-eww-show-status)))

;; (define-key eww-mode-map (kbd "C-g") #'my-eww-keyboard-quit)
(define-key eww-mode-map (kbd "C-g") nil)
(define-key eww-mode-map (kbd "C-t") 'my-eww-show-status)


(defun my-eww-follow-link ()
  (interactive)
...
;; (define-key eww-mode-map (kbd "C-m") 'my-eww-follow-link)
(define-key eww-mode-map (kbd "C-m") nil)
(define-key eww-link-keymap (kbd "C-m") 'my-eww-follow-link)
...
(defun eww-reader (url)
  ...
  (if (re-match-p "^http" url)
      (let ((readerurl (cl-sn (concat "rdrview -url " (q url)) :chomp t)))
        (message readerurl)
...
(defun google-this-url-in-this-domain (url domain)
  ...
  (let ((query (concat "site:" (q domain) " " "intext:" (q url))))
    (eegr query))
  ;; (url-domain (url-generic-parse-url (get-path)))
  )
...
(define-key eww-mode-map (kbd "g") 'eww-reload)
(define-key eww-mode-map (kbd "m") 'toggle-use-chrome-locally)
