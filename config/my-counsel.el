...
;; this just isn't working atm with spacemacs package versions

;; to fix a bug if the var is not available
(if (not (my/function-exists 'ivy-highlight-grep-commands))
    (defset ivy-highlight-grep-commands nil))
(if (not (my/function-exists 'ivy-make-magic-action))
    (defun ivy-make-magic-action (a b) nil))

(require 'counsel)

; vim +/"ivy-height" "$EMACSD/config/my-ivy.el"
...
(defun fz-gl (query)
  ...
  (let ((result (fz (snc (concat (cmd "gl") " " query))
