;;; flymake-elixir-autoloads.el --- automatically extracted autoloads
;;
;;; Code:

(add-to-list 'load-path (directory-file-name
                         (or (file-name-directory #$) (car load-path))))


;;;### (autoloads nil "flymake-elixir" "flymake-elixir.el" (0 0 0
;;;;;;  0))
;;; Generated autoloads from flymake-elixir.el

(autoload 'flymake-elixir-load "flymake-elixir" "\
Configure flymake mode to check the current buffer's elixir syntax." t nil)

(register-definition-prefixes "flymake-elixir" '("flymake-elixir-"))

;;;***

;; Local Variables:
;; version-control: never
;; no-byte-compile: t
;; no-update-autoloads: t
;; coding: utf-8
;; End:
;;; flymake-elixir-autoloads.el ends here
